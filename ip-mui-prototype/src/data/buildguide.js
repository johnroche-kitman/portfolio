...
export const AREAS = [
  ...
  {
    id: 'event-detail',
    name: 'Event detail — session, game and event',
    blurb: 'Three detail pages in the product, one of them built here. A session, a game and a plain event all live at the same route and differ by type: the session has six tabs, the game has six different ones, the event has three. Only the session is built in this prototype — what is known about the other two comes from measuring the live product, and is recorded below so nobody starts from zero.',
    files: [
      ['pages/events/SessionDetail.jsx', 'The session and all six of its tabs, plus the drill panels. 752 lines, the largest file in the prototype.'],
      ['pages/events/EventEditor.jsx', 'Create and edit, shared by all three types. /events/new and /events/:id.'],
      ['pages/events/SessionImporter.jsx', 'The three-step importer the session links out to.'],
      ['data/session.js', 'Tab names, drill fields, principle taxonomies, squad picker data.'],
    ],
    surfaces: [
      {
        title: 'Event editor — create and edit',
        route: '/events/new?type=Game|Session|Event, /events/:id',
        what: 'The full page that creates or edits any of the three types. Reached from Add in the calendar toolbar and from Edit details on a session.',
        mui: ['Paper', 'TextField', 'Autocomplete', 'ToggleButtonGroup', 'ToggleButton', 'RadioGroup', 'Radio', 'FormControl', 'FormLabel', 'FormControlLabel', 'Checkbox', 'Chip', 'Dialog', 'DialogTitle', 'DialogContent', 'DialogActions', 'InputAdornment', 'Divider', 'IconButton', 'Button', 'Box', 'Typography'],
        icons: ['ArrowBack', 'Delete', 'Upload', 'Add'],
        notes: [
          'This is a page, not a drawer. That is deliberate and matches the live product — authoring an event is too large for a panel, and the calendar Add menu routes here rather than opening one.',
          'Type is a `ToggleButtonGroup`, and it drives which fields render. Build the three variants as one form with conditional sections, not three forms: they share squad, date, time, location, recurrence and attachments.',
          'Recurrence is the expensive part. The preset options are a `TextField select`; Custom opens a `Dialog` with an interval field, day-of-week `ToggleButtonGroup`, and a `RadioGroup` for how it ends — never, on a date, or after N times, with N in a `TextField` carrying an `InputAdornment` reading "Times".',
          'Squads, athletes and staff are `Autocomplete multiple` with `Chip`s. Same pattern as the calendar filter rail — reuse it.',
          'Save is the page’s single contained button. Delete is an icon button in the header, not a red button in the form.',
        ],
      },
      {
        title: 'Session detail — shell and tabs',
        route: '/sessions/:id',
        what: 'The six-tab session page: Planning, Athlete selection, Staff selection, Development goals, Collection, Imported data.',
        mui: ['Tabs', 'Tab', 'Breadcrumbs', 'Switch', 'FormControlLabel', 'Button', 'Box', 'Typography', 'Divider'],
        notes: [
          'Tabs are `variant="scrollable"` with `scrollButtons="auto"`. At six tabs it does not scroll on a desktop width, but the game has six longer ones and the medical athlete profile has ten, so the scrollable variant is the house default — do not switch it to fixed.',
          'The Complete toggle in the header is a `Switch`. Turning it on marks the session complete, and that state shows as a tick on the event in all four calendar views. It is the one piece of cross-surface state in the prototype, so wire it to real state early rather than keeping it local to the page.',
          'Edit details appears on every tab and opens the event editor page, not a panel.',
          'The live equivalent is 30.8% MUI on the Planning tab, the heaviest tab in the product at 1,364 elements.',
        ],
      },
      {
        title: 'Planning tab and drill cards',
        route: '/sessions/:id → Planning',
        what: 'The ordered list of drills, each a card with a pitch thumbnail, intensity and activity chips, duration, and inline Add links for area size, note and principles.',
        mui: ['Paper', 'Chip', 'Box', 'Typography', 'IconButton', 'Button'],
        icons: ['DragIndicator', 'MoreVert', 'Add'],
        custom: 'Pitch thumbnail is an inline SVG. Drag-to-reorder has no MUI equivalent.',
        notes: [
          'The pitch diagram is a small hand-drawn SVG, not an image, so it scales and themes. Keep it that way.',
          'Reordering drills is drag-and-drop, which MUI does not provide. Use dnd-kit — it is the smallest option and works with React 18. The `DragIndicator` icon is the handle.',
          'Intensity renders as a `Chip` coloured by level, activity as a plain `Chip`. Both come from the same taxonomy in data/session.js.',
          'The inline "Add: Area size | Note | Principles" row is text buttons, not a menu. They edit the card in place.',
        ],
      },
      {
        title: 'Drill panels',
        route: 'Planning → Add drill from library, Create new drill, a drill card, Principles',
        what: 'Four drawers: the library picker, the create form, drill detail, and the principles picker.',
        mui: ['Tabs', 'Tab', 'Accordion', 'AccordionSummary', 'AccordionDetails', 'List', 'ListItemButton', 'ListItemIcon', 'ListItemText', 'ToggleButtonGroup', 'ToggleButton', 'Checkbox', 'FormControlLabel', 'Chip', 'IconButton', 'Button', 'Box', 'Typography'],
        icons: ['Star', 'StarBorder', 'DragIndicator', 'Add', 'Close'],
        notes: [
          'All four use the shared `AddPanel` shell from components/AddPanel.jsx, which is config-driven: pass a title, an optional chip and a body. Do not build a fifth drawer shell.',
          'The library picker has Squad library / My favourites `Tabs` and groups drills by activity, with a `Star` toggle per row for favourites.',
          'Create new drill nests inside the library picker — it opens from a button inside that drawer, so the two stack. Panels stacking is normal here; make sure closing the top one does not close both.',
          'Drill detail uses `Accordion` for principles, labels and squad visibility, all collapsed by default.',
          'Measured live: the library picker is 0% MUI with none of its 25 buttons MUI, create-drill 0% of 11, drill detail 0% of 19, principles 0% of 5. This whole cluster is a clean rebuild, not a conversion.',
        ],
      },
      {
        title: 'Athlete and staff selection',
        route: '/sessions/:id → Athlete selection, Staff selection',
        what: 'An editable grid of selected athletes with participation level and per-drill in/out toggles, and the drawer that adds or removes them.',
        mui: ['DataGrid', 'GridToolbarContainer', 'GridToolbarColumnsButton', 'Switch', 'FormControlLabel', 'Checkbox', 'Avatar', 'Button', 'Box', 'Typography'],
        notes: [
          'Editing happens in the grid, not a form. Participation is a column with `type: "singleSelect"` and `editable: true`, so the DataGrid supplies the dropdown — do not render your own `Select` in a cell.',
          'Commit edits through `processRowUpdate`, returning the updated row. That is the v7 API; older examples using `onCellEditCommit` will not work.',
          'Per-drill in/out is a `Switch` inside `renderCell`, one column per drill, so the column set is data-driven rather than fixed.',
          'The grid has a custom toolbar built from `GridToolbarContainer` with `GridToolbarColumnsButton` — the column picker matters here because the drill columns can be numerous.',
          'Measured live: Add/remove athletes is 0.1% MUI across 1,975 elements with 130 checkboxes; the staff equivalent is 0% across 937. The game has its own Add Players at 0% across 1,962 — within a few elements of the session one, so treat them as one component and build it once.',
        ],
      },
      {
        title: 'Development goals',
        route: '/sessions/:id → Development goals',
        what: 'Goals attached to the session, added through a drawer.',
        mui: ['Paper', 'Chip', 'Button', 'Box', 'Typography', 'RichTextField (local)'],
        notes: [
          'Uses the shared `AddPanel` again. Nothing bespoke here — this is the cheapest tab on the page.',
        ],
      },
      {
        title: 'Collection',
        route: '/sessions/:id → Collection',
        what: 'Forms collected against the session, with a left rail listing them and the selected form rendered beside it.',
        mui: ['Collapse', 'List', 'ListItemButton', 'ListItemText', 'RadioGroup', 'Radio', 'Checkbox', 'FormControlLabel', 'IconButton', 'Box', 'Typography'],
        icons: ['ChevronLeft', 'ChevronRight'],
        notes: [
          'The rail collapses with `Collapse orientation="horizontal"` and `unmountOnExit`, rather than swapping in a different layout. That keeps the rail’s scroll position and avoids a reflow of the form beside it.',
          'Form questions are rendered from data by type — richtext, multiselect, radio — so adding a question type means adding a branch, not a component.',
          'This is the second-largest surface in iP by element count. Budget for it accordingly.',
        ],
      },
      {
        title: 'Imported data',
        route: '/sessions/:id → Imported data',
        what: 'Rows imported against the session, and the route out to the importer.',
        mui: ['DataGrid', 'Button', 'Box', 'Typography'],
        notes: [
          'Import data does not open a drawer — it navigates to /mass_upload/event_data, the three-step importer. Confirmed on the live product; the drawer in an earlier version of this prototype was wrong.',
          'The importer itself is already MUI in the live product and does not need converting.',
        ],
      },
      {
        title: 'Game detail — not built here',
        route: 'Live: /planning_hub/events/:id (type Game)',
        what: 'Six tabs: Athlete selection, Staff selection, Game events, Development goals, Collection, Imported data. Only the audit measurements exist for it.',
        mui: ['— not built in this prototype —'],
        custom: 'Nothing built. Figures below are measured off the live product.',
        notes: [
          'Athlete selection, Staff selection, Development goals, Collection and Imported data are the same shapes as the session, so most of the session work carries over. Game events is the one genuinely new tab.',
          'Game events has a Pitch view / List view toggle. Pitch view carries score entry, an Add period button, a Period 1–3 timeline marked in minutes to full time, and an Available Players rail with a Line-ups select. Measured at 27.9% MUI.',
          'List view is the heavier of the two at 2,668 elements: a period rail carrying the formation per period, then a grid of athlete against each period with total minutes, participation, group calculations, yellow, red, goal, assist and own goal. 19.9% MUI, and 120 of its 148 inputs are not MUI.',
          'The view toggle is not a `ToggleButtonGroup` in the live product. It should be.',
          'Game details, the game’s own editor, measures 22.1% and is the counterpart to Edit details on a session — build them as one component.',
          'Both measurements were taken on a game with no players selected, so the grids were empty. Treat them as floors: every athlete adds a row of inputs across three periods.',
        ],
      },
      {
        title: 'Event detail — not built here',
        route: 'Live: /planning_hub/events/:id (type Event)',
        what: 'Three tabs: Athletes, Staff, Attachments. The smallest of the three detail pages.',
        mui: ['— not built in this prototype —'],
        custom: 'Nothing built.',
        notes: [
          'All three tabs are shapes already solved by the session: an athlete picker, a staff picker and a file list. If the session is built first, this page is mostly assembly.',
          'Build this one first of the two remaining. It is the cheapest way to prove the shared pieces from the session actually generalise before committing them to the game.',
        ],
      },
    ],
  },
